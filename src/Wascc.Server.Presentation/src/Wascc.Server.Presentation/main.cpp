...
void execute()
{
	listener = new TcpConnectionListener("9090");
	Console::writeLine("Listener was initialized.");

	Console::writeLine("Starting to accept a connection.");
	connection = listener->acceptOne();
...
int __cdecl main1(void)
{
	...
	if (iResult != 0)
	{
		Console::writeLine("WS startup failed with error: " + std::to_string(iResult) + ".");
		return 1;
	}
